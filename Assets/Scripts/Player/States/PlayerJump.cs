...
public class PlayerJump : State
{
    ...
    private float JumpForce => Mathf.Sqrt(2f * ctx.fallAcceleration * ctx.jumpHeight) + (ctx.fallAcceleration * Time.fixedDeltaTime * 0.5f);
    ...
    protected override void OnEnter() => ExecuteJump();

    protected override State GetTransition()
    {
        if (!ctx.grounded && ctx.frameVelocity.y <= 0f)
        {
            return ((PlayerAirborne)Parent).PlayerFall;
        }

        if (!ctx.jumpHeld && ctx.frameVelocity.y > 0f)
        {
